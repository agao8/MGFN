...
MODEL_LOCATION = 'saved_models_performer_x3d/'
MODEL_NAME = '888tiny'
MODEL_NAME2 = None
MODEL_NAME3 = None
...
def test(dataloader, model, args, device = 'cuda', name = MODEL_NAME):
    model.to(device)
    plt.clf()
    with torch.no_grad():
        model.eval()
        ...
        plt.legend()
        plt.savefig(name + '_roc.png', bbox_inches='tight')
        plt.close()
        ...
        plt.legend()
        plt.savefig(name + '_pr.png', bbox_inches='tight')
        plt.close()
